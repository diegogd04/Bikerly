...
class RemoveFavoriteMotorbikeUseCaseTest {
    @Test
    fun `When invoke then remove favorite motorbike`() = runTest {
        val motorbikeRepositoryMock = mockk<MotorbikeRepository>(relaxed = true)
        val removeFavoriteMotorbikeUseCase = RemoveFavoriteMotorbikeUseCase(motorbikeRepositoryMock)
        ...
        coVerify(exactly = 1) { motorbikeRepositoryMock.removeFavoriteMotorbike(motorbike) }
    }
}